<template>
	<section class="min-h-screen w-full" ref="aboutSection" id="about-section">
		...
		<div
			:class="[
				'flex flex-col gap-y-4 xl:grid xl:grid-cols-2 xl:gap-x-5 xl:gap-y-0 mb-36 text-slate-500 dark:text-slate-300 transition-all motion-reduce:transition-none duration-500 delay-300',
				visible
					? 'translate-y-0 opacity-1 blur-0'
					: 'translate-y-4 opacity-0 blur-sm',
			]"
		>
			<div class="order-2 xl:order-none">
				<p v-for="paragraph in content.autobiography">
					{{ paragraph }}<br /><br />
				</p>
				<div>
					<p class="pb-3">Most Recent Tech Stack...</p>
					<ul
						class="flow flow-col list-['\2705'] pl-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4"
					>
						<li v-for="tech in content.techStack" class="pl-2 text-base">
							{{ tech }}
						</li>
					</ul>
				</div>
